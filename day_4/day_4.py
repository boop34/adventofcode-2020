...
def check_validity(fields, mode=1):
    '''
    ...
    mode -> 1 for the first puzzle
         -> anything else for the second puzzle :)
    ...
    for field, value in fields.items():
        if not value and field != 'cid':
            return False

    # for the first puzzle the above check is enough
    if mode == 1:
        return True
    else:
        # check the 'byr'(birth year) field: must be 4 digits and must be
        # within 1920 to 2002
        try:
        ...
        # check the 'iyr'(issue year) field: must be 4 digits and must be
        # within 2010 and 2020
        try:
        ...
        # check the 'pid'(passport id) field: must be a 9 digit number
...
with open('input.txt', 'r') as f:
    ...
    for line in f.readlines():
        ...
        if not line.strip():
            ...
            if check_validity(fields, mode=2):
                valid_count += 1
